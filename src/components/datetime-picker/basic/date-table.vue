<template>
  <table cellspacing="0" cellpadding="0" class="b-date-table" @click="handleClick" @mousemove="handleMouseMove" :class="{ 'is-week-mode': selectionMode === 'week' }">
    <tbody>
      <tr>
        <th v-if="showWeekNumber">周次</th>
        <th v-for="week in WEEKS">{{week}}</th>
      </tr>
      <tr class="b-date-table__row" v-for="row in rows" :class="{ current: isWeekActive(row[1]) }">
        <td v-for="cell in row" :class="getCellClasses(cell)" v-text="cell.type === 'today' ? '今天' : cell.text"></td>
      </tr>
    </tbody>
  </table>
</template>
