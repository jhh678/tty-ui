<template>
  <transition name="b-zoom-in-top" @before-enter="panelCreated" @after-leave="$emit('dodestroy')">
    <div v-show="visible" :style="{ width: width + 'px' }" class="b-time-range-picker b-picker-panel" :class="popperClass">
      <div class="b-time-range-picker__content">
        <div class="b-time-range-picker__cell">
          <div class="b-time-range-picker__header">开始时间</div>
          <div :class="{ 'has-seconds': showSeconds }" class="b-time-range-picker__body b-time-panel__content">
            <time-spinner ref="minSpinner" :show-seconds="showSeconds" @change="handleMinChange" @select-range="setMinSelectionRange" :hours="minHours" :minutes="minMinutes" :seconds="minSeconds">
            </time-spinner>
          </div>
        </div><div class="b-time-range-picker__cell">
          <div class="b-time-range-picker__header">结束时间</div>
          <div :class="{ 'has-seconds': showSeconds }" class="b-time-range-picker__body b-time-panel__content">
            <time-spinner ref="maxSpinner" :show-seconds="showSeconds" @change="handleMaxChange" @select-range="setMaxSelectionRange" :hours="maxHours" :minutes="maxMinutes" :seconds="maxSeconds">
            </time-spinner>
          </div>
        </div>
      </div>
      <div class="b-time-panel__footer">
        <button type="button" class="b-time-panel__btn cancel" @click="handleCancel()">取消</button>
        <button type="button" class="b-time-panel__btn confirm" @click="handleConfirm()" :disabled="btnDisabled">确定</button>
      </div>
    </div>
  </transition>
</template>
