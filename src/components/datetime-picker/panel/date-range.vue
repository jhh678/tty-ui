<template>
  <transition name="b-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div v-show="visible" :style="{ width: width + 'px' }" class="b-picker-panel b-date-range-picker" :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="b-picker-panel__body-wrapper">
        <slot name="sidebar" class="b-picker-panel__sidebar"></slot>
        <div class="b-picker-panel__sidebar" v-if="shortcuts">
          <button type="button" class="b-picker-panel__shortcut" v-for="shortcut in shortcuts" @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="b-picker-panel__body">
          <div class="b-date-range-picker__time-header" v-if="showTime">
            <span class="b-date-range-picker__editors-wrap">
              <span class="b-date-range-picker__time-picker-wrap">
                <b-input
                  size="small"
                  ref="minInput"
                  placeholder="开始日期"
                  class="b-date-range-picker__editor"
                  :value="minVisibleDate"
                  @input.native="handleDateInput($event, 'min')"
                  @change.native="handleDateChange($event, 'min')"></b-input>
              </span>
            <span class="b-date-range-picker__time-picker-wrap">
                <b-input
                  size="small"
                  ...
                  @change.native="handleTimeChange($event, 'min')"></b-input>
                <time-picker
            ...
            <span class="b-date-range-picker__editors-wrap is-right">
              <span class="b-date-range-picker__time-picker-wrap">
                <b-input
                  size="small"
                  placeholder="结束日期"
                  ...
                  @change.native="handleDateChange($event, 'max')"></b-input>
              </span>
            <span class="b-date-range-picker__time-picker-wrap">
                <b-input
                  size="small"
                  ...
                  :readonly="!minDate"
                  @change.native="handleTimeChange($event, 'max')"></b-input>
                <time-picker
          ...
          <div class="b-picker-panel__content b-date-range-picker__content is-left">
            <div class="b-date-range-picker__header">
              <button type="button" @click="prevYear" class="b-picker-panel__icon-btn iconfont icon-first"></button>
              <button type="button" @click="prevMonth" class="b-picker-panel__icon-btn iconfont icon-front"></button>
              <div>{{ leftLabel }}</div>
            </div>
          ...
          <div class="b-picker-panel__content b-date-range-picker__content is-right">
            <div class="b-date-range-picker__header">
              <button type="button" @click="nextYear" class="b-picker-panel__icon-btn iconfont icon-last"></button>
              <button type="button" @click="nextMonth" class="b-picker-panel__icon-btn iconfont icon-next"></button>
              <div>{{ rightLabel }}</div>
            </div>
      ...
      <div class="b-picker-panel__footer" v-if="showTime">
        <a class="b-picker-panel__link-btn" @click="handleClear">清空</a>
        <button type="button" class="b-picker-panel__btn" @click="handleConfirm()" :disabled="btnDisabled">确定</button>
      </div>
    </div>
  </transition>
</template>
