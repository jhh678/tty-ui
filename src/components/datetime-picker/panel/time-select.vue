<template>
  <transition name="b-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div ref="popper" v-show="visible" :style="{ width: width + 'px' }" :class="popperClass" class="b-picker-panel time-select">
      <b-scrollbar noresize wrap-class="b-picker-panel__content">
  ...
  export default {
    ...
    methods: {
      ...
      scrollToOption(className = 'selected') {
        const menu = this.$refs.popper.querySelector('.b-picker-panel__content')
