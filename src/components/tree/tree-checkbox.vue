	...
	<div class="tree" :class="model.permissions.length < 1 ? 'flex' : ''">
        <div class="nodeItem" :class="{bold: isFolder}"
            ...
            :class="model.permissions.length < 1 ? 'visibility' : ''">{{open ? '-' : '+'}}</span>
						<b-checkbox @change="parentCheckAllChange" v-model="parentCheck">全选</b-checkbox>
            {{model.name}}
        </div>
        <ul v-show="open">
            <li class="tree-li" v-for="(model, index) in model.permissions">
              <div class="model-name">{{model.name}}</div>
                <b-checkbox v-model="model.isCheckAll" 
                            ...
                            class="check-class"
                            v-if="model.permissions.length">全选</b-checkbox>
                <b-checkbox v-else
                            v-model="model.isCheckAll" 
                            @change="handleCheckAllChange(index, $event, 'singel')" 
                            class="check-class"></b-checkbox>            
								<b-checkbox-group v-model="model.checkedRoles" :index="index" @change="handleCheckedRolesChange">
                	<b-checkbox v-for="(role, index) in model.permissions.data" :label="role.id" :key="role.name">{{role.name}}</b-checkbox>
              	</b-checkbox-group>
            </li>
        </ul>
	</div>
...
export default {
  ...
  mounted() {
    // 是否全选
    this.parentCheck = this.model.valid ? this.model.valid : this.parentCheck
  },
  methods: {
    toggle: function() {
      if (this.isFolder) {
