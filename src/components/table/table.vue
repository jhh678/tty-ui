/* * @File: 表格组件 * @Author: jhh678 * @Date: 2018-03-28 16:05:39 * @Last Modified by: jhh678 * @Last Modified time: 2018-03-28
16:07:45 */
      ...
      <div class="table-scroll" ref="cScroll" :style="titleFixed == 'fixed' ? {overflow: 'auto', height: scrollHeight + 'px'} : {}">
        ...
        <div class="table-body" :style="titleFixed == 'fixed' ? {paddingTop: titleHeight + 'px'} : ''">
          <table :style="{width: setWithd}" :id="tableId">
            ...
            <tbody>
              <tr v-for="(dataItem, index) in data" :key="index" @mouseenter.stop.prevent="mouseEnter(index)" @mouseout.stop.prevent="mouseOut(index)" v-on:click.stop="tItamClick(dataItem)" :class="toTrClass(index)" :style="{height: tdHeight + 'px'}">
                <td v-if="showSelect" class="tbSelect">
                  <b-checkbox v-model="checkedItems" :label="index" :show-label="false" @change="checkedSingleItem"></b-checkbox>
                </td>
                <td v-if="orderNumber" style="text-align: center">{{calculateOrderNumber(index)}}</td>

                <td v-for="(item, index1) in columns" :key="index1" :class="'text' + item.textAlign" v-ellipsis="dataItem[item.key] + ',' +item.textLine" v-html="showTdContent(dataItem, item, index)" @click="clickTd(dataItem, item)"></td>
...
export default {
  ...
  watch: {
    ...
    'pagination.pageCurrent': function (val) {
